...
xmlns:ns="urn:schemas-microsoft-com:xslt" exclude-result-prefixes="ms ns xsi">-->
	...
	<xsl:template match="ApexHe">
		<c level="series">
			...
			<did>
				...
				<xsl:apply-templates select="HeEksemplarInfo/Eksemplar/AOEpId"/>
			...
			<xsl:apply-templates select="HeEksemplarInfo/Eksemplar/AOScanningStatus"/>
	...
	<xsl:template match="HeInfoBund">
		...
		Bemærkninger
				...
				<p>
					<xsl:apply-templates select="PubBem"/>
				</p>
			</scopecontent>
		</xsl:if>
	</xsl:template>
	...
	<xsl:template match="HeEksemplarInfo/Eksemplar/AOScanningStatus">
		<xsl:if test=".">
			<scopecontent encodinganalog="summary">
				<head>
		Scanning status
	...
	<xsl:template match="HeEksemplarInfo/Eksemplar/AOEpId">
		<xsl:if test=".">
			<dao>
				
				<xsl:attribute name="xlink:href">http://www.sa.dk/ao/billedviser?epid=<xsl:value-of select="."/></xsl:attribute>
				<xsl:attribute name="xlink:title">Læs scannet udgave af arkivserien her</xsl:attribute>
